...
const PlayerExplorerHelp = () => {
  return (
    <Box>
      ...
      <Text my={4} as="p">
        It also supports search query syntax like when you do email search. For
        example, if you want to specifically find a player based on their name
        FPL page you can use field search <code>name</code> like shown below:
      </Text>
      <Box my={4} pointerEvents="none">
      ...
      <Text my={4} as="p">
        You can also do field keyword combination to expand your search results,
        for example, if you want to search for players from Manchester United OR
        Liverpool you can do the following:
      </Text>
      <Box my={4} pointerEvents="none">
        <PlayerSearchBar initialSeachQuery="name:MUN,LIV" />
      </Box>
      <Text my={4} as="p">
        Or if you want to narrow down your search result you could append anoter
        field search query, for example, the following will show Manchester
