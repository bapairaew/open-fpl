...
export const getServerPlayersStats = (
  ...
  live.elements
    .filter((e) => e.explain.some((e) => e.fixture === fixture.id))
    .forEach((e) => {
      ...
      if (player) {
        ...
        const stats = explain
          ? {
              goals_scored:
                explain?.stats.find((s) => s.identifier === "goals_scored")
                  ?.value ?? 0,
              assists:
                explain?.stats.find((s) => s.identifier === "assists")?.value ??
                0,
              yellow_cards:
                explain?.stats.find((s) => s.identifier === "yellow_cards")
                  ?.value ?? 0,
              red_cards:
                explain?.stats.find((s) => s.identifier === "red_cards")
                  ?.value ?? 0,
              bps: bps ?? 0,
              bonus: bonus ?? 0,
              total_points:
                explain?.stats.reduce((sum, s) => sum + s.points, 0) ?? 0,
              minutes:
                explain?.stats.find((s) => s.identifier === "minutes")?.value ??
                0,
            }
          : null;
  ...
  if (!bonusAvailable) {
    const [first, second, third] = [
      // @ts-ignore
      ...new Set(all.map((e) => e.stats?.bps ?? 0)),
